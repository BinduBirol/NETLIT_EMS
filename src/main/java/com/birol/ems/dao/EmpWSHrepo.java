...
public interface EmpWSHrepo extends JpaRepository<Employee_work_schedule, String> {
	...
	@Query(value = "SELECT * from employee_work_schedule  where userId = :userid AND  date = CURDATE() ",nativeQuery = true)
	public ArrayList<Employee_work_schedule> getToday(@Param("userid")long userid);
