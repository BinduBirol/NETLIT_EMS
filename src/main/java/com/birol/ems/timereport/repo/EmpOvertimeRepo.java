...
public interface EmpOvertimeRepo extends CrudRepository<Timereport_Overtime_emp, String>{
	...
	@Query(value = "SELECT * from emp_time_report_overtime  where userid = :userid and date BETWEEN STR_TO_DATE(:startDate, '%Y-%m-%d')  AND STR_TO_DATE(:endDate, '%Y-%m-%d') and isapproved=1 order by week asc, date asc",nativeQuery = true)
